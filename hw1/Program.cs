...
namespace Shapes
{
  ...
  public class Strategize
  {
    ...
    public static Shape[] loadShapes(string fileType, string fileName)
    {
      ...
      if (fileType == "xml") // xml
      {
        System.Xml.Serialization.XmlSerializer reader =
            ...
            new XmlRootAttribute("Shape"));
        StreamReader file = new StreamReader(fileName);
        Shape[] shapes = (Shape[])reader.Deserialize(file);
        ...
        if (shapes.Length == 0)
        {
            throw new System.ArgumentException("There was an error parsing the xml file.");
        }
    ...
    //catch
    //{
    //    throw new System.ArgumentException("There was an error loading the shapes from the " fileType + " file.");
    //}
